...
export async function GET(request: Request) {
  ...
  return Response.json(signals.map((signal) => signalToExportRow(signal, url.origin, manifest)), {
    headers: {
      ...headers,
...
function exportFilters(url: URL) {
  ...
  for (const key of ["q", "state", "office", "race", "type", "status", "since", "ingestedSince", "committee", "minAmount", "position", "targetParty", "targetStatus", "sort"]) {
